...
class OrientServer {
  ...
  }) => _post('/database/$databaseName/${_storage(storage)}/${_type(type)}')
  ...
  /// Sends a HTTP DELETE request to the OrientDB server
  Future _delete(String path) =>
    http.delete('http://$host:$port$path', headers: _auth);

  /// Sends a HTTP DELETE request to the OrientDB server
  Future _get(String path) =>
    http.get('http://$host:$port$path', headers: _auth)
      .then((response) => JSON.decode(response.body));

  /// Sends a HTTP DELETE request to the OrientDB server
  Future _post(String path, [body]) =>
